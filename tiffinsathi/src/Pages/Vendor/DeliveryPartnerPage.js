    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        {trendValue !== undefined && trendValue !== null && !loading && (
          <div
            className={`flex items-center text-sm font-medium ${trendValue > 0 ? "text-green-600" : trendValue < 0 ? "text-red-600" : "text-gray-600"}`}
          >
            {trendValue > 0 ? (
...
const DeliveryPartnersPage = () => {
  ...
  const fetchDeliveryPartners = useCallback(async () => {
    try {
      ...
      if (response.ok && Array.isArray(response.data)) {
        ...
        const active = partnersArray.filter(
          (p) =>
            p.isActive &&
            (p.availabilityStatus === "AVAILABLE" ||
              p.availabilityStatus === "available"),
        ).length;
        const busy = partnersArray.filter(
          (p) =>
            p.isActive &&
            (p.availabilityStatus === "BUSY" ||
              p.availabilityStatus === "busy"),
        ).length;
        const inactive = partnersArray.filter(
          (p) => !p.isActive || p.availabilityStatus === "OFFLINE",
        ).length;

        // Calculate total deliveries and success rate from partner data
        const totalDeliveries = partnersArray.reduce(
          (sum, p) => sum + (p.completedDeliveries || 0),
          0,
        );
        const totalDelivered = partnersArray.reduce(
          (sum, p) => sum + (p.successfulDeliveries || 0),
          0,
        );
        const successRate =
          totalDeliveries > 0
        ...
        const avgRating =
          totalPartners > 0
            ? (
                partnersArray.reduce((sum, p) => sum + (p.rating || 4.0), 0) /
                totalPartners
              ).toFixed(1)
            : 0;
        ...
        try {
          ...
          if (statsResponse.ok && statsResponse.data) {
            ...
            growthRate =
              newPartners > 0
                ? Math.round((newPartners / totalPartners) * 100)
                : 0;
          }
        } catch (err) {
          // Fallback calculation
          growthRate =
            totalPartners > 0 ? Math.round((active / totalPartners) * 100) : 0;
        }
  ...
  // Delete partner - REAL API call (called from confirmation modal)
  const handleDeleteConfirm = async () => {
    if (!deleteConfirmPartnerId) return;
    setDeleteLoading(true);
    try {
      const response = await vendorApi.deleteDeliveryPartner(deleteConfirmPartnerId);
      if (response.ok) {
      ...
      toast.error("Failed to delete delivery partner: " + error.message);
    } finally {
  ...
  const handleResetPassword = async (partnerId, partnerName) => {
    if (
      window.confirm(
        `Are you sure you want to reset ${partnerName}'s password?`,
      )
    ) {
      try {
        const response =
          await vendorApi.resetDeliveryPartnerPassword(partnerId);
        if (response.ok && response.data) {
          toast.success("Password reset successfully");
          // Handle response - could be string or object
          if (typeof response.data === "object" && response.data.tempPassword) {
            ...
            setTempPassword("Check email for new password");
          }
  ...
  const filteredPartners = deliveryPartners.filter((partner) => {
    ...
    if (statusFilter === "inactive")
      return matchesSearch && (status === "inactive" || status === "offline");
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex space-x-2">
                        <button
                          onClick={() =>
                            handleResetPassword(partner.partnerId, partner.name)
                          }
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    ...
                    <p className="mt-3 text-sm text-red-600">
                      ⚠️ This password will not be shown again. Make sure to
                      copy it now!
                    </p>
                  </div>
                </div>
      ...
      <ConfirmationModal
        isOpen={!!deleteConfirmPartnerId}
        onClose={() => setDeleteConfirmPartnerId(null)}
        onConfirm={handleDeleteConfirm}
        title="Delete Delivery Partner"
        message="Are you sure you want to delete this delivery partner? This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
        type="delete"
