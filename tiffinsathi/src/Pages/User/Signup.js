  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.code === "ERR_NETWORK" || error.message === "Network Error") {
        ...
        if (status === 404) {
          errorMessage = `404 Not Found: The endpoint '/auth/signup' was not found. Please check: 1) The endpoint path in your backend controller, 2) If there's a context path (like /api) that needs to be added, 3) Verify the backend route is registered. Check browser console for more details.`;
        } else {
          errorMessage =
